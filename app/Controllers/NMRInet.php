...
class NMRInet extends BaseController
{
    ...
    ')
        ...
        // dekripsi password_inet agar bisa ditampilkan di form
        ...
        foreach ($data['md_nomer_inet'] as &$r) {
            try {
                ...
                $r['password_inet'] = ''; // data lama tidak bisa didekripsi → kosongkan
            }
        }
        unset($r);
    ...
    public function delete($id)
    {
        ...
        try {
            ...
            return redirect()->to('/NMRInet')
                ->with('success', 'Data Nomor Inet berhasil dihapus.');
        } catch (\Throwable $e) {

            // Tulis error ke log agar bisa dicek jika diperlukan
            log_message('error', 'Nomor Inet Delete Error : ' . $e->getMessage());
    ...
    ')
        ...
        // dekripsi password_inet agar bisa ditampilkan di form edit
        $encrypter = \Config\Services::encrypter();
        foreach ($data['md_nomer_inet'] as &$r) {
            try {
                $r['password_inet'] = ($r['password_inet'] !== null && $r['password_inet'] !== '')
    ...
    public function show($id)
    {
        ...
        $data = $db->table('d_nomor_inet dn')
            ...
            v.nama_vendor
        ')
            ->join('md_nomer_inet ni', 'ni.id = dn.nomer_inet_id', 'left')
            ->join('md_layanan_vendor lv', 'lv.id = ni.layanan_vendor_id', 'left')
            ->join('md_vendor v', 'v.id = lv.vendor_id', 'left')
            ->where('dn.id', $id)
            ->get()
        ...
        $encrypter = \Config\Services::encrypter();
        try {
            $data['password_inet'] = ($data['password_inet'] ?? '') !== ''
