...
class RipotRetail extends BaseController
{
    /**
     * Halaman "Task Down" — hanya menampilkan data dengan status = 1 (Down).
     */
    public function index(): string
    {
        $model = new RipotRetailModel();

        $data['repot_noc']  = $model->where('status', 1)->orderBy('id', 'DESC')->findAll();
        $data['pageTitle']  = 'Task Down';
        $data['filterMode'] = 'down';

        return view('RipotRetail/index', $data);
    }

    /**
     * Halaman "Task On Progress" — hanya menampilkan data dengan status = 0
     * atau kosong/null (belum pernah diisi).
     */
    public function progress(): string
    {
        $model = new RipotRetailModel();

        $data['repot_noc'] = $model
            ->groupStart()
            ->where('status', 0)
            ->orWhere('status', null)
            ->groupEnd()
            ->orderBy('id', 'DESC')
            ->findAll();
        $data['pageTitle']  = 'Retail & Corporate';
        $data['filterMode'] = 'progress';

        return view('RipotRetail/progress', $data);
    }

    public function create()
    {
        return view('RipotRetail/FormRipotRetail');
    }

    /**
     * Ambil semua field repot_noc dari request.
     * Input datetime-local formatnya "2026-07-15T10:50" -> ubah T jadi spasi.
     */
    private function getFormData(): array
    {
        $dt = function ($field) {
            $v = $this->request->getPost($field);
            return $v ? str_replace('T', ' ', $v) : null;
        };
        ...
        return [
            ...
            'finish_time'            => $dt('finish_time'),
            'duration_hour'          => $this->request->getPost('duration_hour') ?: null,
            'duration_minute'        => $this->request->getPost('duration_minute') ?: null,
            'jenis_layanan'          => $this->request->getPost('jenis_layanan'),
            'problem'                => $this->request->getPost('problem'),
            'action'                 => $this->request->getPost('action'),
            'status'                 => $this->request->getPost('status'),
            'note'                   => $this->request->getPost('note'),
            'solver'                 => $this->request->getPost('solver'),
            'backup_start_time'      => $dt('backup_start_time'),
            'backup_finish_time'     => $dt('backup_finish_time'),
            'backup_duration_hour'   => $this->request->getPost('backup_duration_hour') ?: null,
            'backup_duration_minute' => $this->request->getPost('backup_duration_minute') ?: null,
            'daily'                  => $this->request->getPost('daily'),
            'regards'                => $this->request->getPost('regards'),
        ];
    }
    ...
    public function save()
    {
        ...
        return redirect()->to(site_url('RipotRetail'))->with('success', 'Data Repot NOC berhasil ditambahkan.');
    }
    ...
    public function delete($id)
    {
        ...
        return redirect()->to('/RipotRetail')->with('success', 'Data berhasil dihapus.');
    }
    ...
    public function update()
    {
        ...
        if (!$model->find($id)) {
            return redirect()->to('/RipotRetail')->with('error', 'Data tidak ditemukan.');
        }

        $model->update($id, $this->getFormData());

        return redirect()->to('/RipotRetail')->with('success', 'Data Repot NOC berhasil diupdate.');
    }
    ...
    public function edit($id)
    {
        ...
        $data['repot_noc'] = $model->find($id);

        if (!$data['repot_noc']) {
            return redirect()->to('/RipotRetail')->with('error', 'Data tidak ditemukan.');
        }

        return view('RipotRetail/EditFormRipotRetail', $data);
    }
    ...
    public function sendEmail()
    {
        ...
        // Header: Action, Solver, Regards, Status — HARUS sama urutannya
        // dengan sel data di bawah, atau kolom akan bergeser.
        ...
        $gangguanTableHtml .= '<tr style="background:#04a9f5;color:#fff">
            ...
            <th>Durasi</th><th>Jenis Layanan</th><th>Problem</th><th>Action</th><th>Solver</th><th>Regards</th><th>Status</th>
        </tr>';
    ...
    /**
     * Badge HTML berwarna untuk status Gangguan (repot_noc) yang dipakai di
     * body email (tabel & narasi) — HIJAU untuk On Progress, MERAH untuk
     * Down. Ini HTML buatan sistem sendiri (bukan dari input pengguna),
     * jadi sengaja TIDAK di-esc() supaya warnanya tampil di email.
     */
