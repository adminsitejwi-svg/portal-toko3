...
<body class="text-[#37474f] dark:text-[#bfc8d6]">
    ...
    <div id="container" class="pc-container ml-sidebar min-h-screen transition-[margin] duration-200">
        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <h5 class="font-medium text-lg">Aktivasi Retail</h5>
                <a href="<?= site_url('RipotActive/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition">
                ...
                <div class="card-body">
                    ...
                    <div class="table-scroll">
                        <table id="mediaKoneksiTable" class="display nowrap" style="width:100%">
                            ...
                            <tbody>
                                <?php if (!empty($aktivasi)) : ?>
                                    <?php $no = 1; ?>
                                    <?php foreach ($aktivasi as $row) : ?>
                                        <tr>
                                            <td><?= $no++; ?></td>
                                            <td>
                                                <a href="<?= site_url('RipotActive/edit/' . $row['id']) ?>" class="btn btn-primary btn-sm">
                                            ...
                                            <td class="col-bold"><?= esc($row['id_pelanggan']); ?></td>
                                ...
                                <div class="p-6 overflow-y-auto">
                                    ...
                                    <div class="border rounded-lg divide-y">
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
                                            ...
                                            <span class="font-medium text-right" id="detail_id_pelanggan">-</span>
                                        </div>
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
                                        ...
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
                                            ...
                                            <span class="font-medium text-right" id="detail_status">-</span>
                                        </div>
                                    </div>
    ...
    <script>
        document.getElementById('btnKirimEmail').addEventListener('click', function() {
            Swal.fire({
                title: 'Kirim Email?',
                text: 'Sistem akan mengambil data tanggal terbaru dari database, lalu mengirimkannya ke email.',
                icon: 'question',
                ...
                fetch("<?= site_url('RipotActive/sendEmail') ?>", {
                        method: 'POST',
                        body: formData
                    })
                    .then(res => res.json())
    ...
    <script>
        function openDetailModal(id) {
            fetch("<?= site_url('RipotActive/show/') ?>" + id)
                .then(res => {
                ...
                .then(json => {
                    ...
                    document.getElementById('detail_ringkas').textContent =
                        'Data Aktivasi Retail ( ID: ' + d.id + ' ) ';

                    document.getElementById('detail_id_pelanggan').textContent = d.id_pelanggan ?? '-';
                    document.getElementById('detail_layanan').textContent = d.layanan || '-';
