...
<head>
    ...
    <style>
        ...
        #invTable tbody td.col-bold {
            font-weight: 600;
            color: #2b3540;
        }

        .dark #invTable tbody td.col-bold {
            color: #e7eaf0;
        }
...
<body class="text-[#37474f] dark:text-[#bfc8d6]">
    ...
    <div id="container" class="pc-container ml-sidebar min-h-screen transition-[margin] duration-200">
        ...
        <div class="p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <h5 class="font-medium text-lg">Inventory dan stok barang kantor lantai 2</h5>
                <a href="<?= site_url('InventoryKantor/create') ?>" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-scroll">
                        <table id="invTable" class="display nowrap" style="width:100%">
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Aksi</th>
                                ...
                                <?php if (!empty($inventory)) : ?>
                                    ...
                                    <?php foreach ($inventory as $row) : ?>
                                        <tr>
                                            ...
                                            <td>
                                                <button
                                                    ...
                                                    onclick="openEditModal(
                                                        '<?= $row['id'] ?>',
                                                        '<?= esc($row['nama_barang'], 'js') ?>',
                                                        '<?= esc($row['jumlah'], 'js') ?>',
                                                        '<?= esc($row['sn_kode_barang'], 'js') ?>',
                                                        '<?= esc($row['kondisi_barang'], 'js') ?>',
                                            ...
                                            <td><?= esc($row['sn_kode_barang']) ?: '-'; ?></td>
                                            <td>
                                                <?php if ($row['kondisi_barang'] === 'Baik') : ?>
                                                    <span class="badge badge-paid">Baik</span>
                                                <?php elseif ($row['kondisi_barang'] === 'Rusak') : ?>
                                                    <span class="badge badge-pending">Rusak</span>
                                                <?php elseif ($row['kondisi_barang'] === 'Hilang') : ?>
                                                    <span class="badge badge-due">Hilang</span>
                                                <?php else : ?>
                                                    <span class="badge badge-service">Perlu Service</span>
                                                <?php endif; ?>
                                            </td>
                        ...
                        <div id="editModal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center p-4">
                            <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl p-6">
                                ...
                                <form action="<?= site_url('InventoryKantor/update') ?>" method="POST" id="editForm">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="id" id="edit_id">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div class="md:col-span-2">
                                            <label>Nama Barang <span class="text-red-500">*</span></label>
                                            <input type="text" id="edit_nama_barang" name="nama_barang" placeholder="Masukkan Nama Barang" class="w-full border rounded-lg p-3" required>
                                        </div>
                                        <div>
                                            <label>Jumlah <span class="text-red-500">*</span></label>
                                            <input type="text" id="edit_jumlah" name="jumlah" inputmode="numeric" pattern="[0-9]*" placeholder="Masukkan Jumlah" class="w-full border rounded-lg p-3" required>
                                        </div>
                                        <div>
                                            <label>Kondisi Barang <span class="text-red-500">*</span></label>
                                            <select id="edit_kondisi_barang" name="kondisi_barang" class="w-full border rounded-lg p-3" required>
                                        ...
                                        <div class="md:col-span-2">
                                            <label>SN / Kode Barang <span class="text-red-500">*</span></label>
                                            <textarea id="edit_sn_kode_barang" name="sn_kode_barang" rows="2" placeholder="Masukkan SN / Kode Barang" class="w-full border rounded-lg p-3" required></textarea>
                                        </div>
                                        <div class="md:col-span-2">
                                            <label>Keterangan <span class="text-red-500">*</span></label>
                                            <textarea id="edit_keterangan" name="keterangan" rows="3" placeholder="Masukkan Keterangan" class="w-full border rounded-lg p-3" required></textarea>
                                        </div>
                                    </div>
                        ...
                        <div id="detailModal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center p-4">
                            <div class="bg-white rounded-xl shadow-xl w-full max-w-xl overflow-hidden">
                                <div class="flex justify-between items-center bg-primary-500 text-white px-5 py-3">
                                    <h3 class="font-semibold flex items-center gap-2">Detail Barang</h3>
                                    <button onclick="closeDetailModal()"><i class="ti ti-x text-xl"></i></button>
                                </div>
                                <div class="p-6">
                                    ...
                                    <div class="border rounded-lg divide-y">
                                        ...
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
                                            ...
                                            <span class="font-medium text-right" id="detail_jumlah">-</span>
                                        </div>
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
                                            <span class="text-gray-500">SN / Kode Barang</span>
                                            <span class="font-medium text-right" id="detail_sn">-</span>
                                        </div>
                                        <div class="flex justify-between px-4 py-2.5 text-sm items-center">
                                            <span class="text-gray-500">Kondisi</span>
                                            <span id="detail_kondisi">-</span>
                                        </div>
                                        <div class="flex justify-between px-4 py-2.5 text-sm">
    ...
    <script>
        ...
        function openDetailModal(id) {
            fetch("<?= site_url('InventoryKantor/show/') ?>" + id)
                .then(res => {
                ...
                .then(json => {
                    ...
                    document.getElementById('detail_ringkas').textContent = 'Data Barang ( ID: ' + d.id + ' )';
                    document.getElementById('detail_nama').textContent = d.nama_barang;
                    document.getElementById('detail_jumlah').textContent = d.jumlah;
                    document.getElementById('detail_sn').textContent = d.sn_kode_barang || '-';
                    ...
                    const kondisiEl = document.getElementById('detail_kondisi');
                    if (d.kondisi_barang === 'Baik') {
                        kondisiEl.innerHTML = '<span class="badge badge-paid">Baik</span>';
                    } else if (d.kondisi_barang === 'Rusak') {
                        kondisiEl.innerHTML = '<span class="badge badge-pending">Rusak</span>';
                    } else if (d.kondisi_barang === 'Hilang') {
                        kondisiEl.innerHTML = '<span class="badge badge-due">Hilang</span>';
                    } else {
                        kondisiEl.innerHTML = '<span class="badge badge-service">Perlu Service</span>';
                    }
        ...
        // ====== EDIT ======
        function openEditModal(id, nama_barang, jumlah, sn_kode_barang, kondisi_barang, keterangan) {
            document.getElementById('edit_id').value = id;
            document.getElementById('edit_nama_barang').value = nama_barang;
            document.getElementById('edit_jumlah').value = jumlah;
            document.getElementById('edit_sn_kode_barang').value = sn_kode_barang;
            document.getElementById('edit_kondisi_barang').value = kondisi_barang;
        ...
        // ====== BATASI JUMLAH: HANYA ANGKA ======
        (function () {
            const jml = document.getElementById('edit_jumlah');
            if (!jml) return;
            ...
            // bersihkan hasil paste / autofill agar hanya angka
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "<?= site_url('InventoryKantor/delete/') ?>" + id;
                }
            });
        }
            ...
            const table = $('#invTable').DataTable({
                ...
                buttons: [{
                    ...
                    buttons: [{
                            ...
                            title: 'Data Inventory dan stok barang kantor lantai 2',
                            ...exportConfig,
                ...
                language: {
                    ...
                    emptyTable: "Data inventory belum tersedia",
